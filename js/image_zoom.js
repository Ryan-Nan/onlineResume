jQuery.fn.fancyZoom = function(options){
  ...
  if ($('#zoom').length == 0) {
    var html = '<div class="round_shade_box" id="zoom"> \
					...
					<a href="#close" class="round_box_close iconfont" id="zoom_close">&#xe613;</a> \
				</div>';
  ...
  function show(e) {
    if (zooming) return false;
		...
		var width       =  content_div.width() + 40;
		var height      =  content_div.height() + 40;
		var d           =  window_size;

      /*console.log("width of image:"+width);*/

      //判断是否超过被选中的图片和窗口侧边之间的最佳距离
      if((d.width-width) < 55){
          var bestDistance = 55 - (d.width-width);
          width = width-bestDistance;
          height = height - bestDistance/2;
      }
		...
		var curTop             = e.pageY;
		var curLeft            = e.pageX;
		// console.log("d.width:"+d.width+" content_div'width:"+width+"; curLeft:"+e.pageX);
